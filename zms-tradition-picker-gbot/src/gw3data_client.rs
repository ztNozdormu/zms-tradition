...
pub trait W3dataClient {
    async fn save_picker_bot_symbols_request(
        mut self,
        request: tonic::Request<GrpcPickerBotRequest>,
    ) -> Result<tonic::Response<GrpcPickerBotResponse>, tonic::Status>;
}
...
impl W3dataClient for W3dataServiceClient<Channel> {
    ...
    async fn save_picker_bot_symbols_request(
        ...
        // W3dataServiceClient::save_picker_bot_symbols(&mut self, request).await
...
pub async fn save_picker_bot_symbols(
    ValidatePickerBotRequest(rest_delta_request): ValidatePickerBotRequest,
) -> Result<Json<GrpcPickerBotResponse>, AppError> {
    ...
    save_piker_symbols_with_client(rest_delta_request, client).await
}

pub async fn save_piker_symbols_with_client(
    rest_delta_request: GrpcPickerBotRequest,
    ...
    let grpc_response = client.save_picker_bot_symbols_request(grpc_request).await?;
...
//     use zms_tradition_proto_grpc_types::generated::{
//         PickerBotResponse as GrpcPickerBotResponse, PickerSymbol as GrpcPickerSymbol,
//     };
//     use zms_tradition_rest_types::rest_types::{
//         PickerBotRequest as RestPickerBotRequest, PickerSymbol as RestPickerSymbol,
//     };
...
//                 let grpc_response = GrpcPickerBotResponse {
...
//         let rest_request = RestPickerBotRequest {
//             strategy_type: "ewo_v7".to_string(),
//         };
